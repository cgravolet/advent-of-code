...
func (a *AdventOfCode2022) Puzzle202216(input string) {
	part1, part2 := solvePuzzle202216Part1(input), solvePuzzle202216Part2(input)
	fmt.Printf("Part 1: %d\n", part1)
	fmt.Printf("Part 2: %d\n", part2)
}
...
type Puzzle202216Cache map[int]map[string]map[int]int

func (c *Puzzle202216Cache) addItem(time int, valve string, bitmask int, pressure int) {
	_, ok := (*c)[time]
...
func (c *Puzzle202216Cache) getItem(time int, valve string, bitmask int) (val int, ok bool) {
...
func parseInputPuzzle202216(input string) (map[string]Valve, int) {
	var index int
...
func solvePuzzle202216Part1(input string) int {
	v, _ := parseInputPuzzle202216(input)
	vm := makeValveMap(v)
	cache := make(Puzzle202216Cache)
	return traverse(vm, v, cache, 30, "AA", 0)
}

func solvePuzzle202216Part2(input string) int {
	v, maplen := parseInputPuzzle202216(input)
	vm := makeValveMap(v)
	cache := make(Puzzle202216Cache)
...
func traverse(vm ValveMap, valves map[string]Valve, cache Puzzle202216Cache, time int, valve string, bitmask int) int {
